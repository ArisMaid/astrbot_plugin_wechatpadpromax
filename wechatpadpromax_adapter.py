...
CONFIG_METADATA = {
    ...
    "webhook_url": {
        ...
        "type": "string",
        "hint": "可选。填写后会直接注册到 WeChatPadProMAX。适合 AstrBot 走反向代理的场景，例如 https://bot.example.com/webhook/wechatpadpromax。",
    },
    "webhook_public_base_url": {
        "description": "webhook 对外基础地址",
        "type": "string",
        "hint": "可选。填写 WeChatPadProMAX 能访问到的 AstrBot webhook 基础地址，例如 http://192.168.1.10:6197。已填写完整 webhook 回调地址时会忽略此项。",
    },
    "webhook_secret": {
        "description": "webhook 密钥",
        "type": "string",
        "hint": "可选。需要与 WeChatPadProMAX 注册的 secret 一致，用于校验 webhook 签名。",
    },
    "webhook_host": {
    ...
    "auto_start_heartbeat": {
        "description": "自动启动心跳",
        "type": "bool",
        "hint": "启用后，适配器启动或重载时会调用 WeChatPadProMAX /Login/AutoHeartBeat。除非明确需要，建议保持关闭。",
    },
    "auto_start_sync": {
    ...
    "diagnostic_latency_log": {
        "description": "记录 webhook 延迟诊断日志",
        "type": "bool",
        "hint": "启用后，适配器会记录消息时间戳到 webhook 收到时间的延迟，用于排查消息传递慢的问题。",
    },
    "message_types": {
        "description": "webhook 消息类型过滤",
        "type": "list",
        "hint": "传给 WeChatPadProMAX /Webhook/Set。除非需要限制事件类型，一般保持 *。",
    },
    "accepted_msg_types": {
    ...
    "unified_webhook_mode": {
        ...
        "type": "bool",
        "hint": "高级选项。启用后 AstrBot 通过 /api/platform/webhook/<uuid> 接收回调，而不是使用插件自己的端口。",
    },
    "webhook_uuid": {
    ...
    "unsupported_component_fallback": {
        ...
        "type": "bool",
        "hint": "启用后，合并转发、普通文件等暂不支持的消息段会作为文本摘要发送，避免静默消失。",
    },
    ...
    "download_inbound_media": {
        "description": "下载并转换入站媒体",
        "type": "bool",
        "hint": "启用后，当 webhook 中没有可直接使用的 URL/base64/path 时，通过 WeChatPadProMAX /Tools/Download* 尝试增强图片、语音、视频和文件消息。",
    },
    "inbound_media_fallback_text": {
        "description": "入站媒体失败时转文本",
        "type": "bool",
        "hint": "媒体无法解析时保留 [voice] 等可见占位文本，避免消息被丢弃。",
    },
    "inbound_media_cache_dir": {
        "description": "入站媒体缓存目录",
        "type": "string",
        "hint": "可选的本地缓存目录，用于保存必须落盘的下载媒体。留空时使用 AstrBot data/temp/wechatpadpromax。",
    },
    "inbound_media_download_section_len": {
        "description": "入站媒体下载分片长度",
        "type": "int",
        "hint": "传给 WeChatPadProMAX 下载接口的 sectionLen。留 0 表示使用 webhook 中报告的完整媒体大小。",
    },
}
...
class WechatPadProMaxAdapter(Platform):
    def __init__(
        ...
        self.metadata = PlatformMetadata(
            name=ADAPTER_NAME,
            description="WeChatPadProMAX 平台适配器。",
